...
async function handleUpdateTextLayers(updates: Array<{ id: string; newText: string }>) {
  for (const update of updates) {
    const node = figma.getNodeById(update.id) as TextNode;
...
async function handleRemoveBackground(apiKey: string, imageData: string) {
  try {
    figma.ui.postMessage({
...
async function handleUpdateImage(imageData: string) {
  const selection = figma.currentPage.selection;
  ...
  try {
    ...
    if (parent && 'insertChild' in parent) {
      const originalIndex = parent.children.indexOf(originalNode as SceneNode);
    ...
    figma.ui.postMessage({
      type: 'update-image-result',
      success: false,
      error: 'Failed to create image layer: ' + (error as Error).message
    });
  }
}
...
async function handleAutoUpdateContent(apiKey: string, prompt: string) {
  const selection = figma.currentPage.selection;
  ...
  if (selection.length > 0) {
    ...
    for (const node of selection) {
      if (node.type === 'TEXT') {
        ...
        const found = (node as ChildrenMixin).findAll(n => n.type === 'TEXT') as TextNode[];
        textNodes.push(...found);
      }
    }
  } else {
    textNodes = figma.currentPage.findAll(node => node.type === 'TEXT') as TextNode[];
  }
...
function handleRenameLayers(pattern: string, layerType: string) {
  const selection = figma.currentPage.selection;

  let nodesToRename: SceneNode[];
  ...
  if (selection.length > 0) {
    ...
    for (const node of selection) {
      ...
      if ('findAll' in node) {
        const found = (node as ChildrenMixin).findAll(n =>
          layerType === 'all' || n.type === layerType
        ) as SceneNode[];
        nodesToRename.push(...found);
      }
    }
  } else {
    nodesToRename = figma.currentPage.findAll(node =>
      layerType === 'all' || node.type === layerType
    ) as SceneNode[];
  }
